...
class TestFreeVigilanceReduction(unittest.TestCase):
    def setUp(self):
        ...
        with open(self.config_path, "w", encoding="utf-8") as f:
            json.dump({}, f)
        self.engine = FreeVigilanceReduction(config_path=self.config_path, regex_path=self.regex_file.name)
    ...
    def test_reduce_text(self):
        text = "Иван Иванович работает в Газпроме."
        report = self.engine.reduce_text(text, profile_id="test_profile")
