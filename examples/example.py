import os
import sys

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from free_vigilance_reduction.core import FreeVigilanceReduction

base_dir = os.path.dirname(__file__)
config_path = os.path.join(base_dir, 'config', 'profiles.json')
print(config_path)
regex_path = os.path.join(base_dir, 'config', 'regex_patterns.json')
input_path = os.path.join(base_dir, 'inputs', 'input.txt')
output_path = os.path.join(base_dir, 'inputs', 'input_redacted.txt')
report_path = os.path.join(base_dir, 'inputs', 'report.json')



fvr = FreeVigilanceReduction(config_path=config_path, regex_path=regex_path)

print("Загруженные профили:", list(fvr.config_manager.profiles.keys()))

report = fvr.process_file(input_path, profile_id="demo_profile")

report.save_to_file(report_path)

with open(output_path, 'w', encoding='utf-8') as f:
    f.write(report.reduced_text)

print("Обработка завершена.")
